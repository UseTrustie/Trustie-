...
1. INTERNAL CONSISTENCY — Do the claims agree with each other? Add up dated roles and compare to the stated years of experience. Check whether tenure claims match the timeline. Flag contradictions (e.g., header says "8+ years" but summary says "7 years"; dated jobs only sum to ~5 years). EDUCATION ORDER: resumes list education most-recent-first, so a Master's/MBA usually appears ABOVE the Bachelor's even though the Bachelor's was earned first -- judge order by GRADUATION YEAR, never by listing position. A Bachelor's in one year followed by a Master's/MBA in a LATER year is the normal, correct progression; NEVER call that a contradiction. Only flag if a higher degree is dated BEFORE the lower degree it requires (e.g., MBA 2022 but Bachelor's 2024). These are HIGH severity.
